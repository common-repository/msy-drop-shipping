...
if( ! class_exists('Msydrop_Shipping_Ord_List') ){

    class Msydrop_Shipping_Ord_List extends WP_List_Table
    {
        ...
        public function column_cb($item)
        {
            ...
            echo '<input type="checkbox" name="order_ids[]" class="order_ids" value="'.esc_attr($item->ID).'">';
        }
        ...
        public function column_default( $item, $column_name )
        {
            switch( $column_name ) {
                ...
                case 'action':
                    return '<span class="dashicons dashicons-trash delete-order" data-order_id="' . $item->ID . '"></span>';
                case 'status':
                    return __('Pending');
                case 'details':
        ...
        public function extra_tablenav($which){
            if($which == "top"){
                $today = date('Ymd');
                $last_sync = get_option('msyds_last_order_sync_date', '');
                if( $last_sync != $today ){
                    echo '<input type="button" id="processSelOrders" class="button" value="Process Selected Orders"> ';
                    echo '<input type="button" id="processAllOrders" class="button" value="Process All Orders">';
                    echo '<div class="order-sync-message"></div>';
                }
            }
        }
